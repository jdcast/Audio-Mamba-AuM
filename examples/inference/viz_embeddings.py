...
AuM = models.AudioMamba(
    ...
    # if_cls_token=False, # necessary to retrieve all token embeddings
    # final_pool_type='all' # necessary to retrieve all token embeddings
)
...
with torch.no_grad():
    all_token_embeddings = AuM(fbank, return_features=True)

# Convert embeddings to CPU and numpy
all_token_embeddings = all_token_embeddings.cpu().numpy()
#
# # Visualize all token embeddings using t-SNE
...
# embeddings_2d = tsne.fit_transform(all_token_embeddings.reshape(-1, all_token_embeddings.shape[-1]))
#
# # Plot the embeddings
# # plt.figure(figsize=(10, 8))
# # plt.scatter(np.squeeze(embeddings_2d[:, 0]), np.squeeze(embeddings_2d[:, 1]), s=5)
# # plt.title('2D Visualization of All Token Embeddings')
# # plt.xlabel('Dimension 1')
# # plt.ylabel('Dimension 2')
# # plt.show()

# Patch Grid Visualization
# Assume output_embeddings is the tensor of shape (B, N, D)
# Specifically, output_embeddings is (1, 512, 768)
...
# Now patch_grid has the shape (1, 8, 64, 768) which represents the embeddings in their original patch layout

# # Assume patch_grid is of shape (B, H_patches, W_patches, D)
B, H_patches, W_patches, D = patch_grid.shape
...
# plt.imshow(reduced_grid, cmap='viridis', aspect='auto')
# plt.colorbar(label='PCA Reduced Value')
# plt.title('Heatmap of Patch Embeddings')
# plt.xlabel('Patch Index (Width)')
# plt.ylabel('Patch Index (Height)')
# plt.show()
#
# # Option 1: Plot Spectrogram with Patch Grid
# plt.figure(figsize=(12, 6))
# plt.imshow(fbank_np, aspect='auto', origin='lower', cmap='viridis')
# plt.colorbar(label='Magnitude')
# plt.title('Spectrogram with Patch Grid')
# plt.xlabel('Time Frames')
# plt.ylabel('Frequency Bins')
#
# # Overlay the grid lines
# for i in range(1, W_patches):
#     plt.axvline(x=i * (fbank_np.shape[1] // W_patches), color='white', linestyle='--', linewidth=0.5)
# for j in range(1, H_patches):
#     plt.axhline(y=j * (fbank_np.shape[0] // H_patches), color='white', linestyle='--', linewidth=0.5)
#
# plt.show()
#
# # Option 2: Plot PCA-Reduced Patch Embeddings as Heatmap
# plt.figure(figsize=(12, 6))
# plt.imshow(reduced_grid, cmap='plasma', aspect='auto', origin='lower')
# plt.colorbar(label='PCA Reduced Value')
# plt.title('Heatmap of PCA-Reduced Patch Embeddings')
# plt.xlabel('Patch Index (Width)')
# plt.ylabel('Patch Index (Height)')
# plt.show()
